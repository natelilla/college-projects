...
/**
 * This is the GridFill class for the multi-thread class without a barrier.
 * <p>
...
public class GridFill {
     ...
     * @param rowsAndColumnds - int that specifies the amount of rows and columns the grid has
     ...
     */
    public static void gridInsert(double[][] grid, int rowsAndColumnds, int topSide, int bottomSide, int rightSide, int leftSide) {

        // Nested for loop that fills the grid with desired values.
        for (int i = 0; i < rowsAndColumnds; i++) {
            for (int j = 0; j < rowsAndColumnds; j++) {
                if (i == 0) {
                    grid[i][j] = topSide;
                } else if (i == rowsAndColumnds - 1) {
                    grid[i][j] = bottomSide;
                } else if (j == 0) {
                    grid[i][j] = leftSide;
                } else if (j == rowsAndColumnds - 1) {
                    grid[i][j] = rightSide;
                } else {
                    grid[i][j] = 0;
                }
